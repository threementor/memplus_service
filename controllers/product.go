...
// @Success 200 {string}
// @Failure 403 id is empty
// @router /:id/submit_trade [post]
func (c *ProductController) SubmitTrade() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	user, err := c.GetUser()
	if err != nil{
	...
	if trade, err := models.SubmitTradeForProduct(id, user); err == nil {
		c.Data["json"] = map[string]interface{}{"data": trade.AsMap(), "success": true}
	} else {
