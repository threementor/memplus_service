...
// Put ...
// @Title Put
// @Description forget the Task
// @Param	id		path 	string	true		"forget klg"
// @Success 200 {object} models.Task
// @Failure 403 :id is not int
// @router /:id/forget [put]
func (c *TaskController) Forget() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	task, err := models.ForgetTask(id)
	if err == nil {
...
// @Failure 403 :id is not int
// @router /:id/soso [put]
func (c *TaskController) Soso() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	task, err := models.SosoTask(id)
	if err == nil {
		c.Data["json"] = map[string]interface{}{"success": true, "data": task}
	} else {
		c.Data["json"] = map[string]interface{}{"success": false, "msg": err.Error()}
...
func (c *TaskController) Remember() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	task, err := models.RememberTask(id)
	if err == nil {
		c.Data["json"] = map[string]interface{}{"success": true, "data": task}
	} else {
		c.Data["json"] = map[string]interface{}{"success": false, "msg": err.Error()}
